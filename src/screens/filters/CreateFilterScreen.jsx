...
import { View, Text, StyleSheet, ScrollView, KeyboardAvoidingView, Platform, Switch, TouchableOpacity, Alert } from 'react-native';
...
export default function CreateFilterScreen({ navigation }) {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            await createFilter({ name: data.name.trim(), keywords: kw, categories: selectedCategories, source: data.source, isActive: data.isActive, maxResults: parseInt(data.maxResults, 10) || 25, dateFrom: data.dateFrom || null, dateUntil: data.dateUntil || null });
            navigation.goBack();
        } catch (err) { Alert.alert('Error', 'Failed to create filter.'); } finally { setIsSubmitting(false); }
    };
        ...
        <KeyboardAvoidingView style={[styles.container, { backgroundColor: colors.background }]} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={100}>
            <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
                <View style={styles.section}>
                    ...
                    <Controller control={control} name="name" rules={{ required: 'Filter name is required', minLength: { value: 2, message: 'Min 2 characters' }, maxLength: { value: 50, message: 'Max 50 characters' } }}
                    ...
                    <Controller control={control} name="keywords" rules={{ required: 'Enter at least one keyword', validate: (v) => v.split(',').map(k => k.trim()).filter(Boolean).length > 0 || 'Enter at least one keyword' }}
                        render={({ field: { onChange, value } }) => <Input label="Keywords" placeholder="transformer, neural network, deep learning" value={value} onChangeText={onChange} error={errors.keywords?.message} multiline numberOfLines={2} />} />
                    <Text style={styles.hint}>Separate multiple keywords with commas</Text>
                </View>
                        ...
                        <View style={styles.sourceRow}>
                            {['arxiv', 'crossref', 'both'].map((src) => (
                                <TouchableOpacity key={src} style={[styles.opt, value === src && styles.optActive]} onPress={() => onChange(src)}>
                                    <Text style={[styles.optText, value === src && styles.optTextActive]}>
                                        {src === 'arxiv' ? 'arXiv' : src === 'crossref' ? 'Crossref' : 'Both'}
                                    </Text>
                                </TouchableOpacity>
                            ))}
                        </View>
                ...
                <View style={styles.section}>
                    ...
                    {showCategories && (
                        <View style={styles.grid}>
                            {ARXIV_CATEGORIES.map((cat) => { const s = selectedCategories.includes(cat.value); return (
                                <TouchableOpacity key={cat.value} style={[styles.chip, s && styles.chipActive]} onPress={() => toggleCategory(cat.value)}>
                                    <Text style={[styles.chipText, s && styles.chipTextActive]}>{cat.value}</Text>
                                </TouchableOpacity>); })}
                        </View>
                    )}
                </View>
                ...
                <View style={styles.section}>
                    <Controller control={control} name="isActive" render={({ field: { onChange, value } }) => (
                        <View style={styles.switchRow}>
                            <View><Text style={styles.switchLabel}>Enable Filter</Text><Text style={styles.switchHint}>Active filters appear in your daily feed</Text></View>
                            <Switch value={value} onValueChange={onChange} trackColor={{ false: '#e2e8f0', true: '#93C5FD' }} thumbColor={value ? '#1B4F72' : '#cbd5e1'} />
                        </View>
                    )} />
                </View>

                <Button title="Create Filter" onPress={handleSubmit(onSubmit)} loading={isSubmitting} disabled={isSubmitting}
                    icon={<Ionicons name="checkmark" size={20} color="#fff" />} style={{ marginTop: 8 }} />
...
const styles = StyleSheet.create({
    ...
    chipTextActive: { color: '#1B4F72', fontWeight: '600' },
